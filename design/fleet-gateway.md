...
> [!NOTE]
> Envoy AI Gateway became [Agent Router][agent-router], an [Agentic AI
> Foundation][aaif] project in September 2026, while this design was in
> progress. This document uses the old name.
...
body. It doesn't authenticate anyone: neither gateway checks who is
...
I propose the `InferenceGateway` run [Envoy AI Gateway][envoy-ai-gateway], the
...
A cluster hosts at most one gateway, so two gateways in a region run on two
clusters. An `InferenceGateway` naming a cluster that already has one doesn't
become ready.
...
metadata:
  name: eu
spec:
  # The InferenceCluster the gateway runs on. Immutable, since moving it would
  # move the gateway's address.
  clusterName: gw-gcp-eu
  # Secrets holding certificates for the names callers use.
  tls:
    certificateRefs:
...
status:
  # The address the gateway answers on, and what its DNS names should point at.
  address: 34.56.129.3
...
`/anthropic/v1/messages`. A service whose backends speak OpenAI is reachable
through both, because the gateway translates an Anthropic request to OpenAI.
Envoy AI Gateway doesn't translate the other way, so a backend that speaks only
Anthropic serves only Anthropic callers. The gateway also serves a `/healthz`
on port 80 that returns 200 while its proxy is running. A geo-routed DNS record
or a fronting edge health-checks that to decide whether an address should be in
rotation.

`tls` is optional. A gateway with it answers on whatever names its certificates
cover, which you point at its address. A gateway without it serves plain HTTP,
and publishes a per-API base URL built from its address in `status.endpoints`.
That shape is useful for trying out Modelplane with minimal dependencies, or if
you intend to front Modelplane with an existing gateway that terminates TLS and
authenticates clients. The caller's hop stays plaintext unless someone supplies
a certificate or terminates TLS in front of the gateway.
...
spec:
  ...
  api:
    ...
    # translates an Anthropic request for an OpenAI backend. Most providers serve
...
spec:
  ...
  # priority is an integer, 0 by default, lower preferred. Traffic moves to a
  # higher-numbered priority as the ones below it lose healthy endpoints, so
  # Together is failover here.
  - name: together
    priority: 1
    selector:
      matchLabels:
...
each gateway that retries on a refused connection, timeout, 503 or 429, and
ejects an endpoint that keeps failing, so traffic shifts down a priority as the
tier above it loses healthy endpoints. A 429 means an endpoint is out of quota
or capacity rather than broken, so it's retried but doesn't count towards
ejection. The gateway retries a request on another endpoint in the same tier,
then in the next, each with its own model name, credential and path, but only
until the first byte reaches the client. A backend that dies mid-stream
...
gateway), `auth.apiKey.secretSelector` picks the Secrets with their keys. Each
is a set of API keys.
...
```
Authorization: Bearer sk-mp-d4e5f6…  (or x-api-key: sk-mp-d4e5f6…)
...
```json
{"caller":"ml-team-assistant","service":"ml-team/assistant","endpoint":"mp-ml-team-51733/assistant-eu-b0e0c-kimi-k2-prod-gke-eu-0-30e4d","served_model":"ml-team/kimi-k2","input_tokens":412,"output_tokens":1180,"status":200}
{"caller":"ml-team-nightly-eval","service":"ml-team/assistant","endpoint":"mp-ml-team-51733/assistant-eu-b0e0c-together-kimi-k2-974d9","served_model":"moonshotai/Kimi-K2-Instruct","input_tokens":8140,"output_tokens":96,"status":200}
```
...
| --- | --- | --- | --- |
| Envoy AI Gateway | Envoy + ext_proc | Agentic AI Foundation, multi-vendor | Chosen |
| agentgateway | Custom Rust | Agentic AI Foundation, Solo.io-led | Closest alternative |
...
| Apache APISIX | nginx/Lua | ASF, vendor-neutral | Rejected |
| Higress | Envoy + Istio, Wasm | CNCF Sandbox, Alibaba-led | Rejected |
| Portkey, Bifrost | TypeScript, Go | Single-vendor, open-core | Rejected |

[agentgateway][agentgateway] is technically the strongest alternative. Solo.io
leads it, while Envoy AI Gateway's maintainers come from five companies. Running
it would also mean a second proxy stack alongside the Envoy we already run
everywhere.
...
substrate the rest of Modelplane uses.
...
Path addressing doesn't work with Envoy AI Gateway. Its ext_proc recognizes a
closed set of inference-API paths, the OpenAI and Anthropic endpoints it serves,
and matches them exactly. It answers any other path with a 404 before it reads
the body. So a per-service path prefix would get that 404, and the ext_proc
would never read its body or find the model to route on. A caller names the
service in the request's `model` field instead.
