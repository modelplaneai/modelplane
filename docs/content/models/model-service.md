...
send nothing before the gateway gives up on it and retries the request on
another endpoint, which also counts against the endpoint's health. They default
to `300s` and `60s`.

Whether a response sends anything early depends on whether the caller streams. A
streamed response starts after prefill, so `idle` bounds time to first token. A
response that isn't streamed sends nothing until it's complete. If any of a
...
regardless of the engine behind it: the gateway serves the Messages API under
`/anthropic/v1`, and a client that speaks it, including Claude Code via
