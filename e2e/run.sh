...
# goes through the InferenceGateway, which does hold a certificate, so none of them
...
# emitting one. Read it off the InferenceGateway's Envoy.
envoy_pod="$(kubectl --context "$WLCTX" -n envoy-gateway-system get pods \
	-l gateway.envoyproxy.io/owning-gateway-name=inference-gateway -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true)"
[ -n "$envoy_pod" ] || {
	echo "verify: could not find the InferenceGateway's Envoy pod" >&2
	cleanup_verify_pods
	exit 1
}
