...
def received(message: email.message.Message) -> dict[str, str]:
    """Header names are case-insensitive, and a proxy is free to re-case what the engine sent."""
...
def call(
    ...
    try:
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            return resp.status, received(resp.headers), resp.read()
    except urllib.error.HTTPError as err:
        with err:
            return err.code, received(err.headers), err.read()
...
class ChatCompletions(unittest.TestCase):
    ...
    def test_streaming_returns_event_stream_frames(self) -> None:
        ...
        self.assertEqual(status, OK)
        self.assertIn("text/event-stream", headers.get("content-type", ""), f"headers: {headers}")
        got = frames(body)
