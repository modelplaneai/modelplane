...
# The hop from an InferenceGateway to a cluster gateway crosses whatever network
...
# arrives without an InferenceGateway's certificate.
_GATEWAY_SCHEME = "https"

# Injected into every engine container so an engine can be started under the
# name Modelplane routes to. Reference it in the engine's args:
#
#   args:
#   - --model=Qwen/Qwen3-8B
...
class Composer:
    ...
    def _child_labels(self, cluster_info: scheduling.Candidate) -> dict[str, str]:
        ...
        InferenceCluster and inherited by everything composed there. A cluster's
        labels beat the deployment's own template labels on a collision, because
        the cluster is the authority on where it is.
        """
