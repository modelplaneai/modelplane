...
The gateway runs on an InferenceCluster, using the Envoy Gateway, Envoy AI
Gateway, cert-manager and trust-manager its serving stack installs.

A cluster hosts at most one InferenceGateway, because the objects it composes
there have fixed names, so a second would overwrite the first. Where two
InferenceGateways name one cluster one wins (see owns_cluster) and the other
reports ClusterAlreadyHasGateway.
...
AIGatewayRoute that compose-model-route composed for a ModelService; and that
...
_CLIENT_TRAFFIC_NAME = "inference-gateway-client-traffic"
...
# AIServiceBackend compose-model-route composes for that endpoint.
...
# Two proxy pods, so one restarting, being evicted or rolling during an upgrade
# doesn't take the gateway down. They prefer separate nodes and zones rather than
# requiring them, since a cluster that only hosts a gateway may have one small
# node, and a disruption budget stops a drain evicting both at once.
_PROXY_REPLICAS = 2

# The labels Envoy Gateway puts on this Gateway's proxy pods, which is what
# spreading them selects on.
_PROXY_POD_LABELS = {
    "gateway.envoyproxy.io/owning-gateway-name": _GATEWAY_NAME,
    "gateway.envoyproxy.io/owning-gateway-namespace": REMOTE_NAMESPACE,
}

# How long a stopping proxy pod keeps serving requests already in flight. It
# matches the ModelService default request timeout, so a response started just
# before a restart can finish unless its service allows longer. Envoy Gateway
# adds five minutes to it for the pod's termination grace period.
_DRAIN_TIMEOUT = "300s"
...
# One usage record per request, attributing its tokens to a caller. Engine
# metrics are per-model, with no caller dimension.
#
...
# would drop the caller from the records that price provider spend.
_USAGE_RECORD = {
    "caller": _md("caller"),
...
def _ip_version(address: str) -> int | None:
    ...
    DNS name, and the two are resolved differently.
    """
    try:
...
def _k8s_object(provider_config: str, manifest: dict, *, ready_when: str | None = None) -> k8sobjv1alpha1.Object:
    ...
    Readiness defaults to SuccessfulCreate, which suits Secrets and other
    objects with no runtime status worth waiting on. ready_when derives it from
    a CEL query over the observed object.
    """
    readiness = (
...
class Composer:
    ...
    def compose(self) -> None:
        ...
        self.compose_client_traffic_policy()
    ...
    def resolve_inputs(self) -> bool:
        ...
        there is nowhere to compose. A missing Secret doesn't gate here (see
        resolve_secrets).
        """
        ...
        api_key = self.xr.spec.auth.apiKey if self.xr.spec.auth else None
        if api_key:
            response.require_resources(
                self.rsp,
                name="caller-secrets",
                ...
                match_labels=dict(api_key.secretSelector.matchLabels),
            )
    ...
    def owns_cluster(self) -> bool:
        ...
        Two gateways on one cluster would compose the same fixed-name objects
        and overwrite each other, so the incumbent wins: whichever already has
        an address keeps it, and a gateway created later reports
        ClusterAlreadyHasGateway. Taking the cluster over would delete the
        incumbent's Gateway and bring its load balancer back on a different
        address.
    ...
    def resolve_secrets(self) -> None:
        ...
        A missing Secret keeps the gateway from reporting ready (see
        derive_conditions) but doesn't gate the composition: withdrawing the
        Gateway to wait for it would delete its load balancer and move the
        address callers use. A missing TLS Secret leaves the HTTPS listener
        unprogrammed; missing caller keys leave the auth policy denying every
        request, see compose_caller_auth.
        """
        if self.xr.spec.auth:
            # Sorted by name because order decides which key wins: when two
            # Secrets hold the same caller name, Envoy Gateway keeps the one
            # listed first and silently skips the rest. Unsorted, the winner
            # would depend on the order the API server happens to list them in.
            self.caller_secrets = sorted(
                request.get_required_resources(self.req, "caller-secrets"),
    ...
    def compose_envoy_proxy(self) -> None:
        """An EnvoyProxy configuring this gateway's proxy pods and its
        ...
        from the records that attribute provider spend.
        """
        resource.update(
                    ...
                    "spec": {
                        "provider": {
                            ...
                            "kubernetes": {
                                ...
                                "envoyDeployment": {
                                    "replicas": _PROXY_REPLICAS,
                                    "patch": {"type": "StrategicMerge", "value": _NDOTS_PATCH},
                                    "pod": {
                                        "topologySpreadConstraints": [
                                            {
                                                ...
                                                "labelSelector": {"matchLabels": _PROXY_POD_LABELS},
                                            }
                        ...
                        "shutdown": {"drainTimeout": _DRAIN_TIMEOUT},
                        "telemetry": {
                            "accessLog": {
                                "settings": [
                                    {
    ...
    def compose_gateway(self) -> None:
        ...
        Always an HTTP listener, so a gateway without a certificate still
        answers, which is the getting-started shape and the shape behind someone
        else's edge. An HTTPS listener joins it when the XR carries TLS, and
        inference then binds to it alone so credentials never cross :80 in the
        clear; :80 keeps only /healthz and a redirect (see compose_redirect).
    ...
    def compose_caller_auth(self) -> None:
        ...
        key, which it strips so it travels no further. A caller sends its key in
        Authorization, as "Bearer <key>" the way an OpenAI client does, or in
        x-api-key the way an Anthropic client does. The filter tries them in
        that order and strips both.
    ...
    def compose_failover_policy(self) -> None:
        """A BackendTrafficPolicy that retries a failed request, and ejects an
        endpoint that keeps failing.

        A ModelService's priority is stamped as an endpoint locality priority,
        and Envoy moves traffic to the next priority as the one above loses
        healthy endpoints. Nothing health checks an endpoint actively, so
        ejection is what marks one unhealthy; without it a failing endpoint
        keeps its share of traffic.

        numAttemptsPerPriority decides where a retry goes. Each priority gets
        two attempts before retries skip it, and Envoy Gateway always avoids a
        host that already failed, so the first retry tries another endpoint at
        the same priority and the second moves down. One transient failure on
        one replica then doesn't send a request to a paid backup, and a failing
        priority still costs only one extra attempt.
                    ...
                    "spec": {
                        ...
                        "retry": {
                            ...
                            "retryOn": {
                                ...
                                # is silently inert. A provider answering 503 or
                                # 429, the cases this exists for, would not be
                                ...
                                # A 429 is an endpoint out of quota or shedding
                                # load, so retrying it elsewhere is how a full
                                # tier overflows. Envoy never ejects for one,
                                # because ejection only counts 5xx, so the
    ...
    def compose_client_traffic_policy(self) -> None:
        """A ClientTrafficPolicy that lets a whole request or response body fit
        in the proxy's buffer.

        AI Gateway's ext-proc buffers the whole request body to read the model
        from it, and the whole response body when it isn't streamed. Envoy
        answers 413 once a body outgrows the connection's buffer limit, which
        Envoy Gateway defaults to 32KiB, too small for a long prompt or a
        non-streamed completion. Over HTTP/2 the stream and connection flow
        control windows, 64KiB and 1MiB by Envoy Gateway's default, cap a body
        too, so they're raised as well. The sizes are the ones AI Gateway's
        documentation gives.
        """
        resource.update(
            self.rsp.desired.resources["client-traffic-policy"],
            _k8s_object(
                self.pc,
                {
                    "apiVersion": "gateway.envoyproxy.io/v1alpha1",
                    "kind": "ClientTrafficPolicy",
                    "metadata": {"name": _CLIENT_TRAFFIC_NAME, "namespace": REMOTE_NAMESPACE},
                    "spec": {
                        "targetRefs": [
                            {
                                "group": "gateway.networking.k8s.io",
    ...
    def write_status(self) -> None:
        ...
        The URLs are built from the address, so they're published only for a
        gateway serving plain HTTP. A gateway serving HTTPS is reached on the
        DNS names its certificates cover, which only its owner knows.
        """
        ...
        if address and not self.xr.spec.tls:
    ...
    def shared_caller_key(self) -> tuple[str, str] | None:
        """The first two callers holding the same key, as <secret>/<caller>.

        Envoy Gateway rejects the whole caller policy when two callers share a
        key. The rejection reaches the gateway's readiness only once
        provider-kubernetes observes the policy again, which for a policy that
        was accepted before is its slow drift poll, so every request would be
        refused for minutes while the gateway reported ready. This finds it as
        soon as the Secret is selected, the way Envoy Gateway does: Secrets in
        order and callers sorted within each, where a repeated caller name is
        skipped before its key is compared.
        """
        callers: set[str] = set()
        keys: dict[str, str] = {}
        for secret in self.caller_secrets:
            name = secret["metadata"]["name"]
            for caller, key in sorted(secret.get("data", {}).items()):
                if caller in callers:
                    continue
                if key in keys:
                    return keys[key], f"{name}/{caller}"
                callers.add(caller)
                keys[key] = f"{name}/{caller}"
        return None
    ...
    def derive_conditions(self) -> None:
        ...
        with a 500 while its Gateway still has an address.

        A missing input Secret is reported here (see resolve_secrets). The
        Gateway's own readiness only tracks having an address, so neither a
        fail-closed auth policy nor an unprogrammed HTTPS listener would
        otherwise show.
        """
        ...
        if missing_tls:
            ...
            return
        shared = self.shared_caller_key()
        if shared:
            self.not_ready(
                CONDITION_REASON_AUTH_NOT_ACCEPTED,
                f"Caller {shared[1]} has the same key as {shared[0]}, so Envoy Gateway rejects the caller "
                "authentication policy and every request is refused",
            )
            return
        waiting = [
            key
