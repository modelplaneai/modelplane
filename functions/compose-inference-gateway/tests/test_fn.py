...
class TestFunctionRunner(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_minimal_gateway(self) -> None:
        ...
        self.assertEqual(
            log["spec"]["forProvider"]["manifest"]["spec"]["provider"],
            {
                "type": "Kubernetes",
                "kubernetes": {
        ...
        self.assertEqual(log["spec"]["forProvider"]["manifest"]["spec"]["shutdown"], {"drainTimeout": "300s"})
        self.assertEqual(
    ...
    async def test_full_gateway(self) -> None:
        ...
        self.assertEqual(resource.struct_to_dict(got.desired.composite.resource)["status"], {"address": _ADDRESS})
        self.assertEqual(
            list(got.conditions),
            [
                fnv1.Condition(
    ...
    async def test_endpoints_are_built_from_the_address(self) -> None:
    ...
    async def test_a_shared_caller_key_is_not_ready_before_the_policy_is_observed(self) -> None:
        ...
        for name, secrets, want in cases:
            with self.subTest(name):
                ...
                got = await self.runner.RunFunction(req, None)
                self.assertEqual(
                    [json_format.MessageToDict(c) for c in got.conditions],
                    [json_format.MessageToDict(want)],
                )

    async def test_caller_secrets_are_listed_in_name_order(self) -> None:
        ...
        self.assertEqual(
            policy["spec"]["forProvider"]["manifest"]["spec"]["apiKeyAuth"]["credentialRefs"],
            [{"name": "callers-team-a-keys"}, {"name": "callers-team-b-keys"}],
        )
