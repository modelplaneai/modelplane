...
compose-model-route composes them once per gateway that serves a ModelService
selecting this endpoint. An endpoint can't know that set without reading the
services that select it, and having two XRs compose the same object would put
them in a fight over it.
...
will find it, and lets compose-model-route leave a broken endpoint out of the
