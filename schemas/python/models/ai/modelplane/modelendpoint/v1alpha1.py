...
class Spec(BaseModel):
    api: Api | None = Field({}, validate_default=True)
    """
    ...
    """
    Secret holding this backend's credential, which the gateway attaches on the way out. The credential never reaches the caller, and the caller's own credential never reaches the backend. An endpoint whose Secret is missing carries no traffic and reports EndpointReady=False.
    """
    ...
    A caller names a ModelService and gets back whichever model actually served: ask for ml-team/assistant and the response names the model that answered, such as Qwen/Qwen3-8B.
    """
